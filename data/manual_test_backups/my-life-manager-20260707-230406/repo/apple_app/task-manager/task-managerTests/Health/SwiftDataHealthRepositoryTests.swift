...
struct SwiftDataHealthRepositoryTests {
    ...
    @Test @MainActor func healthRepositorySearchesAndPersistsCustomFoodCatalogItems() throws {
        let repository = try makeRepository()
        ...
        try repository.saveFoodCatalogItem(customFood)

        let allCustomFoods = try repository.fetchCustomFoodCatalogItems()
        let searchResults = try repository.searchFoodCatalogItems(matching: "carrot pea", limit: 10)

        #expect(allCustomFoods == [customFood])
        #expect(searchResults.contains(where: { $0.id == customFood.id }))
        #expect(searchResults.first?.source == .custom)
    }
    ...
    @Test @MainActor func healthRepositoryFetchesByDayAndRecentHistory() throws {
        ...
        let todayMeal = MealLog(timestamp: today, summary: "Soup")
        let oldMeal = MealLog(timestamp: yesterday, summary: "Pasta")
    ...
    @Test @MainActor func healthRepositoryDeletesLogs() throws {
        ...
        let customFood = FoodCatalogItem(name: "Protein oatmeal")
