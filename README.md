**All agents should always read [AGENTS.md](AGENTS.md) and [docs/README.md](docs/README.md) before using this repository.**
...
Read these in order when you need to scope a change quickly:

1. `AGENTS.md`
2. this `README.md`
3. `docs/README.md`
4. `docs/testing_workflow.md`
5. the matching domain doc in `docs/domains/`
6. the matching feature note beside the SwiftUI code, if one exists

Then inspect the smallest relevant implementation surface:

- `apple_app/task-manager/task-manager/App/AppContainer.swift` for the dependency graph
- `apple_app/task-manager/task-manager/ContentView.swift` for the tab shell
- `apple_app/task-manager/task-manager/Features/Home/HomeView.swift` for the main execution surface
- `apple_app/task-manager/task-manager/Models/` for shared domain types
- `apple_app/task-manager/task-manager/Persistence/` for repository contracts and SwiftData storage

If you are changing behavior, update the relevant docs in the same change instead of leaving repo guidance stale.

Verification guidance:

- Prefer build-only verification unless the change truly requires runtime UI testing.
- Do not repeatedly retry simulator-based commands when the machine reports no available iPhone Simulator runtimes.
- Treat CoreSimulator/actool runtime outages as environment limits, not as a reason to keep burning tokens on the same command.
...
- The repo is intentionally documentation-heavy so agents can scope work without reading every Swift file first.
- Several modules are implemented enough to have models, repositories, views, and tests.
- Some domains are still incomplete or intentionally lightweight: Health, Music Practice, Fitness, Shopping, People Memory, Vices, Debriefs, Calendar Block Focus, and Sync should be treated as evolving surfaces rather than finished consumer features. Debriefs are currently parked in a temporarily deactivated state on the Home surface while the code remains usable.
- Product docs in `docs/` describe intended direction and frequently go beyond current UX polish. `docs/README.md` is the fastest entry point into that doc set.
...
- Planner, Debriefs, Block Focus, Banners, Finance, Shopping, Health, Fitness, Music Practice, People Memory, and Vices are surfaced from Home flows or Settings rather than each owning a top-level tab.
...
- Capture Review now lives in a dedicated inbox-review feature. Raw captures are routed through a manifest-driven intake registry so each destination can expose its own inline form, templates, and optional fields instead of sharing one universal conversion form.
- Debriefs still have a queue-style review flow in code. Pending debriefs are presented one at a time, quick outcomes gate a pushed detail screen, and finishing or skipping immediately advances to the next pending item, but the Home widget entry point is currently deactivated.
...
- Finance now has a local-only income and expense dashboard with category-driven entry, monthly balance summaries, and transaction history.
- Banners now provide local notification templates from Settings with urgency, privacy, enable/disable, fixed-time or random-window scheduling, app-wide notification preferences, and route handoff into multiple app modules.
...
- Start from the smallest feature folder that matches the user request.
- If the behavior crosses screens, follow the repository and view-model first, not the SwiftUI body.
- If a change touches Home, expect cross-module summaries to matter even when the edited screen looks local.
- If a change touches Planner or EventKit, verify the source-of-truth boundary before editing UI.
- If a change touches Debriefs, Vices, Routines, or Capture Review, check the matching domain doc because those flows often have hidden handoff rules.
...
- `apple_app/task-manager/task-manager/App/AppNotificationRouteSheet.swift`
...
- `AppNotificationCoordinator.swift` handles local notification presentation, snooze actions, and route handoff into the app.
- `AppNotificationRouteSheet.swift` renders the destination launched from a notification tap once the coordinator has resolved the route.
...
- `Banners`: manual local notifications surfaced from Settings, with routine-first templates, fixed-time and random-window scheduling, urgency/privacy controls, app-wide defaults, and notification route handoff into app modules.
...
- `Debriefs`: app-owned reflection records with a queue-style quick-review flow, task-outcome capture for work blocks, multi-source template inference, and detailed prompts when needed. The feature is temporarily deactivated on the Home surface but remains in the codebase.
...
- `Banners`: manual local notifications with route handling and scheduling logic isolated outside SwiftUI views.
...
- `Capture Review`: a shared sticky-note inbox that can route captures into tasks, project ideas, project notes, shopping items, practice pieces, and people-memory entries.
...
- Banners, including manual local notification templates, random-window scheduling, and notification-route handling
...
- `CaptureModels.swift`
  - shared raw capture, intake-manifest, review draft, and capture module metadata types
- `CaptureCapabilityService.swift`
  - manifest-driven inbox-intake registry plus per-destination validation and persistence providers
...
- Capture Review now lives in `Features/InboxReview/InboxReviewView.swift` rather than inside `HomeView.swift`.
- The shared intake registry exposes manifest-defined module tiles, per-kind templates, and optional-field customization instead of hardcoded candidate sheets.
- Current supported destinations are task, project idea, project note, shopping item, practice piece, and people-memory entry.
- Task creation still reuses `MyTaskFormData`; the other destinations persist from the shared inbox draft through their own providers.
...
- Finance dashboard, summary, and repository behavior
- Calendar Block Focus repository behavior and focus-context plumbing
...
- `docs/README.md`
  - documentation hub and fastest reading order
- `docs/life_assistant_vision.md`
  - broader product framing and current module inventory
- `docs/product_direction.md`
  - frozen product responsibilities and source-of-truth rules
- `docs/iphone_product_scope.md`
  - historical iPhone-specific framing
...
- `apple_app/task-manager/task-manager/Features/*FeatureNotes.md`
  - short implementation notes for Home, Inbox Review, Logs, Practice, Reflection, and Routines
...
4. Read the domain doc in `docs/` that matches the change.
5. Find the existing model, repository, and view model before editing SwiftUI.
6. Make the smallest correct change.
7. Prefer existing patterns over new abstractions.
8. Run the narrowest useful verification.
9. Update docs when behavior, structure, or repo shape changed.
10. If the task is still ambiguous, say which assumption you used rather than silently broadening scope.
...
- Home, Inbox Review, and widget work should usually include the Home / Inbox / Widgets checks in `docs/testing_workflow.md`.
...
If you only changed docs, use the narrowest check that proves the docs still match the repo. That is often a file read or tree refresh rather than a build.
...
│       │   │   └── AppNotificationRouteSheet.swift
│       │   ├── Assets.xcassets
...
│       │   ├── Banners
│       │   │   ├── AlertRouteCoordinator.swift
│       │   │   ├── AlertScheduler.swift
│       │   │   ├── AlertTriggerPlanner.swift
│       │   │   └── RoutineAlertTemplateFactory.swift
│       │   ├── ContentView.swift
│       │   ├── Features
│       │   │   ├── Banners
